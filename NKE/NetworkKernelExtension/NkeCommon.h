...
// DBG for debug
// _NKE_LOG - for log
// _NKE_LOG_ERRORS - for error log
//
#if defined(DBG)
    #define NKE_INVALID_POINTER_VALUE ((long)(-1))
    #define NKE_DBG_MAKE_POINTER_INVALID( _ptr ) do{ (*(long*)&_ptr) = NKE_INVALID_POINTER_VALUE; }while(0);
    #define NKE_IS_POINTER_VALID( _ptr ) ( NULL != _ptr && NKE_INVALID_POINTER_VALUE != (long)_ptr )
#else//DBG
    #define NKE_DBG_MAKE_POINTER_INVALID( _ptr )  do{void(0);}while(0);
    #define NKE_IS_POINTER_VALID( _ptr ) ( NULL != _ptr )
#endif//DBG
...
// the macro uses the internal NkeLog if available or the Apple System Logger( ASL ) in the other case
//
#define NKE_COMM_LOG_EXT( _S_ ) do{\
            IOLog("NKE: %s %s(%u):%s: ", "", __FILE__ , __LINE__, __PRETTY_FUNCTION__ );\
    }while(0);
...
//
#define NKE_COMM_LOG_EXT_TO_ASL( _S_ ) do{\
    IOLog("NKE: %s %s(%u):%s: ", "", __FILE__ , __LINE__, __PRETTY_FUNCTION__ );\
}while(0);
...
//
 ...
 //
 #define DBG_PRINT( _S_ )  do{ bool  isError = false; NKE_COMM_LOG_EXT( _S_ ); }while(0);
...
// errors log
//
#if !defined(_NKE_LOG_ERRORS)

    #define DBG_PRINT_ERROR( _S_ )   do{ void(0); }while(0);//DBG_PRINT( _S_ )

#else

    #define DBG_PRINT_ERROR( _S_ )   do{ NKE_COMM_LOG_EXT( _S_ ); }while(0);
...
//
    ...
    #define DBG_PRINT_ERROR_TO_ASL( _S_ )   do{ NKE_COMM_LOG_EXT_TO_ASL( _S_ ); }while(0);
...
//
#define NKE_COMM_LOG( _TYPE_ , _S_ ) do{ NKE_COMM_LOG_EXT( _S_ ); }while(0);

//
// ASL common log
//
#define NKE_COMM_LOG_TO_ASL( _TYPE_ , _S_ ) do{ NKE_COMM_LOG_EXT_TO_ASL( _S_ ); }while(0);
...
#define NKE_STATIC_ARRAY_SIZE( _ARR_ ) ( (unsigned int)( sizeof(_ARR_)/sizeof(_ARR_[0] ) ) )
...
//--------------------------------------------------------------------
     ...
     */
    #define NKE_MACOSX_10_5 (0x1)
#endif//

#ifdef NKE_MACOSX_10_5
