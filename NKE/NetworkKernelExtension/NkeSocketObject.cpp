 ...
 * Copyright (c) 2012 Slava Imameev. All rights reserved.
 ...
 Some comments and ideas have been borrowed from Apple's tcplognke
...
#include "NkeSocketObject.h"
#include "NkeSocketFilter.h"

//--------------------------------------------------------------------

//
// ATTENTION! we are in a test mode !!!!!
//
// #define _NKE_SOCKET_FILTER_USER_EMULATION

//--------------------------------------------------------------------

#define super OSObject

OSDefineMetaClassAndStructors( NkeSocketObject, OSObject )

//--------------------------------------------------------------------

SInt32          NkeSocketObject::SocketObjectsCounter = 0x0;
bool            NkeSocketObject::Initialized = false;
NkeSocketObject::NkeSocketsListHead NkeSocketObject::SocketsList;
IORWLock*       NkeSocketObject::SocketsListLock;
NkeSocketObject::NkeSocketsListToReportHead NkeSocketObject::SocketsListToReport;
IORWLock*       NkeSocketObject::SocketsListToReportLock;
OSMallocTag		NkeSocketObject::gOSMallocTag;
SInt32          NkeSocketObject::gSocketSequence = 0x1;
...
errno_t NkeSocketObject::InitSocketObjectsSubsystem()
{
    ...
    error = kernel_thread_start ( ( thread_continue_t ) &NkeSocketObject::InjectionThreadRoutine,
                                  NULL,
                                  &thread );
    ...
    NkeSocketObject::Initialized = true;
...
NkeSocketObject::InjectionThreadRoutine( void* context )
{
    do{
        ...
        (void)msleep( &dummyEvent,                  // wait channel
                      NULL,                          // mutex
                      PUSER,                         // priority
                      "NkeSocketObject::InjectionThreadRoutine()", // wait message
                      &ts );                         // sleep interval
    ...
    IOReturn RC = userClient->clientMemoryForType( kt_NkeNotifyTypeSocketFilter, &options, &memoryDescriptor );
...
#error "this is an emulation for a user space notification acceptor, should not be used if a user space acceptor is present"
        ...
        while( kIOReturnSuccess == IODataQueueDequeue( dataQueue, &notification, &dataSize ) ){
            ...
            if( NkeSocketFilterEventDataIn == notification.event || NkeSocketFilterEventDataOut == notification.event ){
        ...
        IORWLockRead( NkeSocketObject::SocketsListLock );
        { // start of the lock
            
            NkeSocketObject*  sockObj;
            
            TAILQ_FOREACH( sockObj, &NkeSocketObject::SocketsList, socketListEntry)
            {
        ...
        IORWLockUnlock( NkeSocketObject::SocketsListLock );
        ...
        while( ! TAILQ_EMPTY( &localSocketsList ) ){
            ...
            //
            sockObj->reinjectDeferredData( NkeSocketDataAll );
        ...
        //
        NkeSocketObject::DeliverWaitingNotifications();
...
void NkeSocketObject::RemoveSocketObjectsSubsystem()
{
    assert( 0x0 == NkeSocketObject::SocketObjectsCounter );
    
    if( NkeSocketObject::SocketsListLock ){
        
        IORWLockFree( NkeSocketObject::SocketsListLock );
        NkeSocketObject::SocketsListLock = NULL;
    }
    
    if( NkeSocketObject::SocketsListToReportLock ){
        
        IORWLockFree( NkeSocketObject::SocketsListToReportLock );
        NkeSocketObject::SocketsListToReportLock = NULL;
    }
...
bool NkeSocketObject::init()
{
    assert( NkeSocketObject::Initialized );
    
    TAILQ_INIT( &this->pendingQueue );
    TAILQ_INIT( (NkeSocketsListHead*)&this->socketListEntry );
...
/*
 ...
 #0  NkeSocketObject::free (this=0xba6b200) at /work/DL_MacSvn/mac/dl-0.x.beta/mac/dl-0.x/NKEriver/NKE/NkeSocketObject.cpp:190
 ...
 #3  0x471715fc in NkeSocketFilter::FltDetach (cookie=0xba6b200, so=0x5d9bc84) at /work/DL_MacSvn/mac/dl-0.x.beta/mac/dl-0.x/NKEriver/NKE/NkeSocketFilter.cpp:364
 #4  0x4717163a in NkeSocketFilter::FltDetachIPv4 (cookie=0xba6b200, so=0x5d9bc84) at /work/DL_MacSvn/mac/dl-0.x.beta/mac/dl-0.x/NKEriver/NKE/NkeSocketFilter.cpp:374
...
void NkeSocketObject::free()
{
    ...
    OSDecrementAtomic( &NkeSocketObject::SocketObjectsCounter );
...
NkeSocketObject* NkeSocketObject::withSocket(
    __in socket_t so,
    __in mbuf_tag_id_t gidtag,
    __in sa_family_t sa_family
    )
{
    NkeSocketObject*   socketObj;
    
    socketObj = new NkeSocketObject();
    assert( socketObj );
    if( ! socketObj ){
        
        DBG_PRINT_ERROR(("socketObj = new NkeSocketObject() failed\n"));
        return NULL;
    }
    
    OSIncrementAtomic( &NkeSocketObject::SocketObjectsCounter );
    ...
    socketObj->capturingMode = NkeCapturingModeAll; // by default capture all new connections' data
    socketObj->socketId.socket = (UInt64)so;
    socketObj->socketId.socketSequence = OSIncrementAtomic( &NkeSocketObject::gSocketSequence );
...
void
NkeSocketObject::insertInSocketsList()
{
    assert( 0x0 == this->flags.insertedInSocketsList );
    assert( NkeSocketObject::SocketObjectsCounter > 0x0 );
    assert( preemption_enabled() );
    assert( ! NkeIsSocketObjectInList( this->socket ) );
    
    this->LockExclusive();
    IORWLockWrite( NkeSocketObject::SocketsListLock );
    { // start of the lock
        
        if( 0x0 == this->flags.insertedInSocketsList ){
            
            TAILQ_INSERT_HEAD( &NkeSocketObject::SocketsList,
                               this,
                               socketListEntry );
...
void
NkeSocketObject::removeFromSocketsList()
{
    ...
    { // start of the lock
        ...
        if( this->flags.insertedInSocketsList ){
            ...
            TAILQ_REMOVE( &NkeSocketObject::SocketsList,
                          this,
                          socketListEntry );
    ...
    IORWLockWrite( NkeSocketObject::SocketsListToReportLock );
    { // start of the lock
        
        if( this->insertedInSocketsListToReport ){
            
            TAILQ_REMOVE( &NkeSocketObject::SocketsListToReport, this, socketsListToReportEntry );
            this->insertedInSocketsListToReport = false;
            wasInSocketsToReportList = true;
        } // end if( ! TAILQ_EMPTY( &NkeSocketObject::SocketsListToReport ) )
    }// end of the lock
    IORWLockUnlock( NkeSocketObject::SocketsListToReportLock );
...
NkeSocketObject*
NkeSocketObject::GetSocketObjectRef( __in socket_t so )
/*
...
void NkeSocketObject::LockShared()
{
    assert( this->rwLock );
    assert( preemption_enabled() );
...
void NkeSocketObject::UnlockShared()
{
    assert( this->rwLock );
    assert( preemption_enabled() );
    assert( NULL == this->rwLockExclusiveThread );
...
void NkeSocketObject::LockExclusive()
{
    assert( this->rwLock );
    assert( preemption_enabled() );
...
void NkeSocketObject::UnlockExclusive()
{
    assert( this->rwLock );
    assert( preemption_enabled() );
...
void NkeSocketObject::logSocketPreEvent( __in sflt_event_t event )
{
...
void NkeSocketObject::logSocketPostEvent( __in sflt_event_t event )
{
...
NkeSocketObject::PendingPktQueueItem*
NkeSocketObject::allocatePkt(
    ...
    )
{
    ...
    //
    assert( NkeSocketObject::gOSMallocTag );
    pkt = (NkeSocketObject::PendingPktQueueItem*)OSMalloc( sizeof(*pkt), NkeSocketObject::gOSMallocTag );
    assert( pkt );
    if( ! pkt )
        return NULL;
...
/*
    @typedef NkeSocketObject::FltDataOut
...
{
    return FltData( so, to, data, control, flags, NkeSocketDataDirectionOut );
}
...
{
    return FltData( so, from, data, control, flags, NkeSocketDataDirectionIn ); 
}
...
/*
    ...
    @typedef NkeSocketObject::FltData
 ...
 #0  0x470d3ced in NkeSocketObject::FltData (this=0xb72ab00, so=0x5dacc24, addr=0x0, data=0x2c1ebaa4, control=0x0, flags=0, direction=NkeSocketDataDirectionIn) at /work/DL_MacSvn/mac/dl-0.x.beta/mac/dl-0.x/NKEriver/NKE/NkeSocketObject.cpp:1043
 #1  0x470d429a in NkeSocketObject::FltDataIn (this=0xb72ab00, so=0x5dacc24, from=0x0, data=0x2c1ebaa4, control=0x0, flags=0) at /work/DL_MacSvn/mac/dl-0.x.beta/mac/dl-0.x/NKEriver/NKE/NkeSocketObject.cpp:828
 #2  0x470d1079 in NkeSocketFilter::FltDataIn (cookie=0xb72ab00, so=0x5dacc24, from=0x0, data=0x2c1ebaa4, control=0x0, flags=0) at /work/DL_MacSvn/mac/dl-0.x.beta/mac/dl-0.x/NKEriver/NKE/NkeSocketFilter.cpp:627
...
{
	errno_t   error = 0x0;
    bool      isInboundData = ( NkeSocketDataDirectionIn == direction );
    NkeSocketObject::WaitEntry waitEntry;
    
    assert( preemption_enabled() );
    assert( NkeSocketDataDirectionOut == direction || NkeSocketDataDirectionIn == direction );
    ...
    if( NkeCapturingModeNothing == this->capturingMode ){
        ...
        // NkeCapturingModeAll but have not yet inserted pending packets so the pending packets
        ...
        //
        if( NkeSocketDataDirectionOut == direction ){
            
            //
            // we can simply wait in the caller's context
            //
            this->checkForInjectionCompletion( NkeSocketDataAll, true );
            return 0x0;
            
        } else {
            
            assert( NkeSocketDataDirectionIn == direction );
            ...
            //
            if( this->checkForInjectionCompletion( NkeSocketDataAll, false ) ){
    ...
    } // end if( NkeCapturingModeNothing == this->capturingMode )
    
    //
	// check whether we have seen this packet previously 
    //
	if( this->checkTag( data, this->gidtag, NKE_SOCKTAG_ID_TYPE, isInboundData ? NKE_INBOUND_DONE : NKE_OUTBOUND_DONE ) ){
        //
		// we have processed this packet previously since out or in tag was attached.
        // bail on further processing
        //
        NKE_COMM_LOG ( NET_PACKET_FLOW, ("bypass so: 0x%p, mbuf_t: 0x%p , isInboundData = %d\n", so, *data, (int)isInboundData ));
		return 0x0;
	}
	//
	...
	NKE_COMM_LOG ( NET_PACKET_FLOW, ("so: 0x%p data: 0x%p pending, bytes %d, isInboundData = %d\n",
    ...
    //
	error = this->setTag( data, this->gidtag, NKE_SOCKTAG_ID_TYPE, isInboundData ? NKE_INBOUND_DONE : NKE_OUTBOUND_DONE );
    assert( KERN_SUCCESS == error );
	if( KERN_SUCCESS == error )
	{
        NkeSocketObject::PendingPktQueueItem*  pendingPkt;
		...
		if( NULL != pendingPkt ){
            ...
            INIT_SOCKET_NOTIFICATION( notification,
                                      this,
                                      isInboundData ? NkeSocketFilterEventDataIn : NkeSocketFilterEventDataOut );
                    ...
                    if( 0x0 == OSIncrementAtomic( &this->packetsWaitingForReporting ) ){
                        ...
                        { // start of the lock
                            ...
                            if( ! this->insertedInSocketsListToReport ){
                                ...
                                // insert in the list
                                //
                                TAILQ_INSERT_TAIL( &NkeSocketObject::SocketsListToReport, this, socketsListToReportEntry );
            ...
            //
            NKE_DBG_MAKE_POINTER_INVALID( pendingPkt );
            ...
            if( sendNotification ){
                ...
                if( userClient ){
                    ...
                    gSocketFilter->releaseUserClient();
                    NKE_DBG_MAKE_POINTER_INVALID( userClient );
...
NkeSocketObject::DeliverWaitingNotifications()
{
    ...
    while( ! TAILQ_EMPTY( &NkeSocketObject::SocketsListToReport ) ){
        
        NkeSocketObject*  sockObj = NULL;
        ...
        { // start of the lock
            ...
            if( ! TAILQ_EMPTY( &NkeSocketObject::SocketsListToReport ) ){
                
                //
                // remove the first object from the list
                //
                sockObj = TAILQ_FIRST( &NkeSocketObject::SocketsListToReport );
                TAILQ_REMOVE( &NkeSocketObject::SocketsListToReport, sockObj, socketsListToReportEntry );
        ...
        { // start of the lock
            ...
            TAILQ_FOREACH( pendingPkt, &sockObj->pendingQueue, pendingQueueEntry)
            {
                ...
                if( pendingPkt->data ){
                    ...
                    INIT_SOCKET_NOTIFICATION( notification,
                                              sockObj,
                                              (( pendingPkt->dataInbound ) ? NkeSocketFilterEventDataIn : NkeSocketFilterEventDataOut) );
                ...
                { // start of the test
                    ...
                    //
                    NkeSocketObject::PendingPktQueueItem*	pendingPktTmp;
        ...
        if( reinsertInList ){
            ...
            { // start of the lock
                ...
                if( reinsertInList ){
                    ...
                    //
                    TAILQ_INSERT_TAIL( &NkeSocketObject::SocketsListToReport, sockObj, socketsListToReportEntry );
        ...
        if( ! reinsertInList ){
            ...
            //
            sockObj->release();
            NKE_DBG_MAKE_POINTER_INVALID( sockObj );
        } // end if( ! reinsertInList )
    ...
    } // end while( ! TAILQ_EMPTY( &NkeSocketObject::SocketsListToReport ) )
}
...
/*
 ...
 #12 0x470c2ae2 in NkeSocketObject::reinjectDeferredData (this=0xbbb2e00, injectType=NkeSocketDataAll) at /work/DL_MacSvn/mac/dl-0.x.beta/mac/dl-0.x/NKEriver/NKE/NkeSocketObject.cpp:1563
 #13 0x470c54b0 in NkeSocketObject::InjectionThreadRoutine (context=0x0) at /work/DL_MacSvn/mac/dl-0.x.beta/mac/dl-0.x/NKEriver/NKE/NkeSocketObject.cpp:256
 ...
 #5  0x00486eef in msleep (chan=0xbbb2e30, mtx=0x0, pri=50, wmsg=0x470e2a68 "NkeSocketObject::acquireDetachingLockForRemoval()", ts=0x31dcbd08) at /SourceCache/xnu/xnu-1504.7.4/bsd/kern/kern_synch.c:335
 #6  0x470c125a in NkeSocketObject::acquireDetachingLockForRemoval (this=0xbbb2e00) at /work/DL_MacSvn/mac/dl-0.x.beta/mac/dl-0.x/NKEriver/NKE/NkeSocketObject.cpp:1937
 #7  0x470c1117 in NkeSocketFilter::FltDetach (cookie=0xbbb2e00, so=0x5dad40c) at /work/DL_MacSvn/mac/dl-0.x.beta/mac/dl-0.x/NKEriver/NKE/NkeSocketFilter.cpp:415
 #8  0x470c117a in NkeSocketFilter::FltDetachIPv4 (cookie=0xbbb2e00, so=0x5dad40c) at /work/DL_MacSvn/mac/dl-0.x.beta/mac/dl-0.x/NKEriver/NKE/NkeSocketFilter.cpp:432
    ...
    )
{
    ...
    // if( !this->acquireDetachingLock() ){
    //    
    //    NKE_COMM_LOG ( NET_PACKET_FLOW,( "acquireDetachingLock() failed for a socket 0x%p\n", this->so ));
    //    return;
    // }
    ...
    { // start of the injection lock
        ...
        { // start of the lock
            ...
            NkeSocketObject::PendingPktQueueItem*	pendingPkt;
            NkeSocketObject::PendingPktQueueItem*	pendingPktNext;
            ...
            for( pendingPkt = TAILQ_FIRST( &this->pendingQueue ); pendingPkt != NULL; pendingPkt = pendingPktNext ){
                ...
                //
                if( NkeSocketDataAll != injectType ){
                    
                    if( (true == pendingPkt->dataInbound && NkeSocketDataInbound != injectType) ||
                        (false == pendingPkt->dataInbound && NkeSocketDataOutbound != injectType))
                        continue;
                    
                } // end if( NkeInjectAll != injectType )
        ...
        while( ! TAILQ_EMPTY( &packetsToInject ) ){
            ...
            if( 0x0 != error ){
                ...
                NKE_COMM_LOG ( NET_PACKET_FLOW, ("error calling sock_inject_data_in/out, so = 0x%p, error = %d, inbound = %d\n",
...
void
NkeSocketObject::setDeferredDataProperties(
    __in NkeSocketDataProperty*  property
    )
{
    assert( preemption_enabled() );
    ...
    { // start of the lock
        ...
        //
        bool shouldBeApplied = ( NkeSocketDataPropertyTypePermission == property->type &&
        ...
        TAILQ_FOREACH( pendingPkt, &this->pendingQueue, pendingQueueEntry )
        {
            ...
            assert( NkeSocketDataPropertyTypeUnknown != property->type );
            switch( property->type ){
                    
                case NkeSocketDataPropertyTypePermission:
...
void
NkeSocketObject::verifyPendingPacketsQueue( __in bool lock )
//
    ...
    IOLockLock( this->injectionMutex );
    {
        ...
        { // start of the lock
            ...
            for( pendingPkt = TAILQ_FIRST(&this->pendingQueue); pendingPkt != NULL; pendingPkt = pendingPktNext )
            {
                ...
                //
                if( NkeSocketDataAll != purgeType ){
                    
                    if( (true == pendingPkt->dataInbound && NkeSocketDataInbound != purgeType) ||
                       (false == pendingPkt->dataInbound && NkeSocketDataOutbound != purgeType))
                        continue;
                    
                } // end if( NkeInjectAll != purgeType )
                ...
                if( pendingPkt->dataInbound ){
                    NKE_COMM_LOG ( NET_PACKET_FLOW, ("****INBOUND PACKET FREED FROM PENDING QUEUE socket_t is 0x%p, mbuf is 0x%p\n", this->socket, pendingPktNext->data));
                } else {
                    NKE_COMM_LOG ( NET_PACKET_FLOW, ("****OUTBOUND PACKET FREED FROM PENDING QUEUE socket_t is 0x%p, mbuf is 0x%p\n", this->socket, pendingPktNext->data));
                }
                ...
                OSFree( pendingPkt, sizeof(*pendingPkt), NkeSocketObject::gOSMallocTag );
                NKE_DBG_MAKE_POINTER_INVALID( pendingPkt );
            } // end for
...
bool
NkeSocketObject::checkForInjectionCompletion( __in NkeSocketObject::NkeSocketDataDirectionType  injectType, bool waitForCompletion )
{
    ...
    if( NkeSocketDataOutbound == injectType || NkeSocketDataAll == injectType ){
        ...
        if( waitForCompletion ){
            ...
            while( this->numberOfPendingOutPackets != 0 ) { // wait for injection completion
                ...
                (void)msleep( &this->numberOfPendingOutPackets,             // wait channel
                              NULL,                          // mutex
                              PUSER,                         // priority
                              "NkeSocketObject::checkForInjectionCompletion()", // wait message
                              &ts );                         // sleep interval
    ...
    } // end if( NkeSocketDataOutbound == injectType || ...
    
    
    if( NkeSocketDataInbound == injectType || NkeSocketDataAll == injectType ){
    ...
    } // end if( NkeSocketDataInbound == injectType || ...
...
void
NkeSocketObject::wakeupWaitingFotInjectionCompletion()
{
...
bool
NkeSocketObject::checkTag(
    __in mbuf_t *m,
    __in mbuf_tag_id_t module_id,
    __in mbuf_tag_type_t tag_type,
    __in NKE_PACKETPROCFLAGS value
    )
{
    errno_t                 error;
    NKE_PACKETPROCFLAGS		*tag_ref;
    size_t                  len;
...
{	
	errno_t                 error;
	NKE_PACKETPROCFLAGS		*tag_ref = NULL;
	size_t                  len;
		...
		if( KERN_SUCCESS == error ){
			...
			*tag_ref = value;
			...
			if( (flags & MBUF_PKTHDR) == 0 ){
                ...
                //
				NKE_COMM_LOG( NET_PACKET_FLOW, ("mbuf_t = 0x%p missing MBUF_PKTHDR bit\n", m));
 ...
 */
errno_t
NkeSocketObject::prependMbufHdr( __inout mbuf_t *data, __in size_t pkt_len)
{
	mbuf_t      new_hdr;
	errno_t     error;
...
bool
NkeSocketObject::acquireDetachingLock()
{
...
void
NkeSocketObject::releaseDetachingLock()
{
...
void
NkeSocketObject::acquireDetachingLockForRemoval()
{
    //
    ...
    do { // wait for any existing client invocations to return
        ...
        (void)msleep( &this->lockCount,      // wait channel
                      NULL,                          // mutex
                      PUSER,                         // priority
                      "NkeSocketObject::acquireDetachingLockForRemoval()", // wait message
                      &ts );                         // sleep interval
...
void
NkeSocketObject::setRemoteAddress( __in const struct sockaddr *remoteAddress )
{
...
void NkeSocketObject::getRemoteAddress( __inout NkeSocketObjectAddress* address )
{
...
void
NkeSocketObject::setLocalAddress( __in const struct sockaddr *localAddress )
{
...
void NkeSocketObject::getLocalAddress( __inout NkeSocketObjectAddress* address )
{
