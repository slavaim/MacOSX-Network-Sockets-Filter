...
#ifndef _NKESOCKETOBJECT_H
#define _NKESOCKETOBJECT_H
...
#define NKE_SOCKTAG_ID_TYPE         0x1

//
// values to use with the memory allocated by the tag function, to indicate which processing has been
// performed already
//
typedef enum NKE_PACKETPROCFLAGS{
	NKE_INBOUND_DONE = 1,
	NKE_OUTBOUND_DONE
} NKE_PACKETPROCFLAGS;

typedef enum NkeSocketDataDirection{
    NkeSocketDataDirectionOut = 1,
    NkeSocketDataDirectionIn
} _NkeSocketDataDirection;
...
class NkeSocketObject: public OSObject{
    
    OSDeclareDefaultStructors( NkeSocketObject );
...
private:
    ...
    //
    static TAILQ_HEAD( NkeSocketsListHead, NkeSocketObject ) SocketsList;
    TAILQ_ENTRY(NkeSocketObject)   socketListEntry;
    ...
    //
    TAILQ_ENTRY(NkeSocketObject)   injectionSocketListEntry;
    ...
    //
    static TAILQ_HEAD( NkeSocketsListToReportHead, NkeSocketObject ) SocketsListToReport;
    TAILQ_ENTRY(NkeSocketObject)   socketsListToReportEntry;
...
private:
    ...
    //
    NkeCapturingMode            capturingMode;
...
public:
    ...
    typedef enum _NkeSocketDataDirectionType{
        NkeSocketDataAll = 0x0,
        NkeSocketDataInbound,
        NkeSocketDataOutbound
    } NkeSocketDataDirectionType;
    
    void purgePendingQueue( __in NkeSocketDataDirectionType purgeType );
...
public:
    ...
    static NkeSocketObject* withSocket( __in socket_t so, __in mbuf_tag_id_t gidtag, __in sa_family_t sa_family );
    ...
    static NkeSocketObject* GetSocketObjectRef( __in socket_t so );
    ...
    void reinjectDeferredData( __in NkeSocketDataDirectionType  injectType );
    
    bool checkForInjectionCompletion( __in NkeSocketDataDirectionType  injectType, __in bool waitForCompletion );
    ...
    void setDeferredDataProperties( __in NkeSocketDataProperty*  property );
    
    void setRemoteAddress( __in const struct sockaddr *remote );
    void getRemoteAddress( __inout NkeSocketObjectAddress* address );
    
    void setLocalAddress( __in const struct sockaddr *local );
    void getLocalAddress( __inout NkeSocketObjectAddress* address );
    ...
    void getSocketId( __inout NkeSocketID* outSocketId ){ *outSocketId = this->socketId; };
...
inline
bool 
NkeIsSocketObjectInList( __in socket_t so )
{
    NkeSocketObject* soObj;
    soObj = NkeSocketObject::GetSocketObjectRef( so );
    if( soObj )
        soObj->release();
...
//
inline
NkeSocketObject* NkeCookieToSocketObject( __in void* cookie )
{
    NkeSocketObject*   sockObj = (NkeSocketObject*)cookie;
...
inline
void* NkeSocketObjectToCookie( __in NkeSocketObject* sockObj )
{    
    assert( SOCKET_OBJECT_SIGNATURE == sockObj->signature );
