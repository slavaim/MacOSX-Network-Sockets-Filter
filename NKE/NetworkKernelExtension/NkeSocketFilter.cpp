...
#define NKE_SOCK_FLT_HANDLE_IP4  0xFEBCD987
#define NKE_SOCK_FLT_HANDLE_IP6  0xFEBCD789

//--------------------------------------------------------------------

#define super OSObject

OSDefineMetaClassAndStructors( NkeSocketFilter, OSObject )

NkeSocketFilter*  NkeSocketFilter::CurrentFilter = NULL;

mbuf_tag_id_t	NkeSocketFilter::gidtag;

//--------------------------------------------------------------------

errno_t NkeSocketFilter::InitSocketFilterSubsystem()
{
    errno_t  error;
...
NkeSocketFilter* NkeSocketFilter::withDefault()
{
    NkeSocketFilter*  newFilter;
    
    newFilter = new NkeSocketFilter();
    assert( newFilter );
    if( ! newFilter ){
        
        DBG_PRINT_ERROR(("new NkeSocketFilter() failed\n"));
        return NULL;
    }
    ...
    //
    newFilter->dataBuffers = OSArray::withCapacity( kt_NkeSocketBuffersNumber );
    ...
    for( int i = 0x0; i < NKE_STATIC_ARRAY_SIZE( newFilter->freeBuffers ); ++i ){
    ...
    for( int i = 0x0; i < newFilter->dataBuffers->getCapacity(); ++i ){
        ...
        if( ! dataBuffer ){
            ...
            DBG_PRINT_ERROR(( "NkeDataBuffer::withSize() failed\n" ));
            break;
        }
...
errno_t NkeSocketFilter::startFilter()
{
    ...
    //
    if( ! OSCompareAndSwapPtr( NULL, this, &NkeSocketFilter::CurrentFilter ) ){
	...
	status = sflt_register( &NkeSocketFilter::SfltIPv4, PF_INET, SOCK_STREAM, IPPROTO_TCP );
	if( KERN_SUCCESS == status ){
        
		this->filterForIPv4IsRegistered = true;
        NKE_COMM_LOG(COMMON,("TCP/IPv4 filter has been registered\n"));
        
	} else {
        
		DBG_PRINT_ERROR(("sflt_register( &NkeSocketFilter::SfltIPv4, PF_INET, SOCK_STREAM, IPPROTO_TCP ) fauled with a status = %d\n",status));
    }
	
	status = sflt_register( &NkeSocketFilter::SFltIPv6, PF_INET6, SOCK_STREAM, IPPROTO_TCP );
	if( KERN_SUCCESS == status ){
        
		this->filterForIPv6IsRegistered = true;
        NKE_COMM_LOG(COMMON,("TCP/IPv6 filter has been registered\n"));
        
    } else {
        
        DBG_PRINT_ERROR(("sflt_register( &NkeSocketFilter::SFltIPv6, PF_INET, SOCK_STREAM, IPPROTO_TCP ) fauled with a status = %d\n",status));
    }
...
errno_t NkeSocketFilter::stopFilter()
{
    errno_t    error = KERN_SUCCESS;
    
    if( this != NkeSocketFilter::CurrentFilter ){
    ...
    if( this->filterForIPv4IsRegistered ){
        ...
        NKE_COMM_LOG(COMMON,("TCP/IPv4 filter has been unregistered\n"));
    ...
    if( this->filterForIPv6IsRegistered ){
        ...
        NKE_COMM_LOG(COMMON,("TCP/IPv6 filter has been unregistered\n"));
    ...
    NkeSocketFilter::CurrentFilter = NULL;
...
void
NkeSocketFilter::FltUnregisteredIPv4(sflt_handle handle)
{
}
...
void
NkeSocketFilter::FltUnregisteredIPv6(sflt_handle handle)
{
}
...
errno_t
NkeSocketFilter::FltAttach(void **cookie, socket_t so, sa_family_t sa_family)
{
    //assert( ! NkeIsSocketObjectInList( so ) );
    ...
    //
    if( NkeIsSocketObjectInList( so ) ){
        
        NkeSocketObject* oldSoObj;
        oldSoObj = NkeSocketObject::GetSocketObjectRef( so );
        if( oldSoObj ){
            
            NkeSocketFilter::FltDetach( NkeSocketObjectToCookie( oldSoObj ), so );
            oldSoObj->release();
            NKE_DBG_MAKE_POINTER_INVALID( oldSoObj );
        }
    }
    
    NkeSocketObject* soObj;
        
    soObj = NkeSocketObject::withSocket( so, NkeSocketFilter::gidtag, sa_family );
    assert( soObj );
    if( soObj ){
        ...
        DBG_PRINT_ERROR(("NkeSocketObject::withSocket failed\n"));
        return ENOMEM;
    }
    
    *cookie = NkeSocketObjectToCookie( soObj );
...
errno_t
NkeSocketFilter::FltAttachIPv4(void **cookie, socket_t so)
{
...
errno_t
NkeSocketFilter::FltAttachIPv6(void **cookie, socket_t so)
{
...
void	
NkeSocketFilter::FltDetach(void *cookie, socket_t so)
{
    NkeSocketObject* soObj = NkeCookieToSocketObject( cookie );
    
    assert( NkeIsSocketObjectInList( so ) && soObj->toSocket() == so );
    ...
    soObj->retain();
    {
        ...
        //
        soObj->purgePendingQueue( NkeSocketObject::NkeSocketDataAll );
        
        //
        // wait for all pending and purging operations completion
        //
        soObj->checkForInjectionCompletion( NkeSocketObject::NkeSocketDataAll, true );
    }
    soObj->release();
    NKE_DBG_MAKE_POINTER_INVALID( soObj );
...
void	
NkeSocketFilter::FltDetachIPv4(void *cookie, socket_t so)
{
    FltDetach( cookie, so );
}
...
void	
NkeSocketFilter::FltDetachIPv6(void *cookie, socket_t so)
{
    FltDetach( cookie, so );
}
...
void	
NkeSocketFilter::FltNotify(void *cookie, socket_t so, sflt_event_t event, void *param)
{
    /*
    ...
    /*
     ...
     #0  NkeSocketObject::logSocketEvent (this=0x8796200, event=4) at /work/DL_MacSvn/mac/dl-0.x.beta/mac/dl-0.x/NKEriver/NKE/NkeSocketObject.cpp:300
     #1  0x47183cf8 in NkeSocketFilter::FltNotify (cookie=0x0, so=0x8cdf000, event=4, param=0x0) at /work/DL_MacSvn/mac/dl-0.x.beta/mac/dl-0.x/NKEriver/NKE/NkeSocketFilter.cpp:268
    ...
    /*
     ...
     #2  0x468e9ce3 in NkeSocketFilter::FltNotify (cookie=0x0, so=0x5215990, event=9, param=0x0) at /work/DL_MacSvn/mac/dl-0.x.beta/mac/dl-0.x/NKEriver/NKE/NkeSocketFilter.cpp:268
    ...
    NkeSocketObject* soObj = NkeSocketObject::GetSocketObjectRef( so );
    if( soObj ){
        ...
        switch( event ){
            ...
            case sock_evt_connected:
            {
                ...
                //
                /*
                 #0  NkeSocketFilter::FltNotify (cookie=0x8d50300, so=0x5e08060, event=2, param=0x0) at /work/DL_MacSvn/mac/dl-0.x.beta/mac/dl-0.x/NKEriver/NKE/NkeSocketFilter.cpp:599
            ...
            case sock_evt_closing:
            {
                ...
                //
                soObj->purgePendingQueue( NkeSocketObject::NkeSocketDataInbound );
                ...
                if( userClient ){
                    ...
                    INIT_SOCKET_NOTIFICATION( notification, soObj, NkeSocketFilterEventClosing );
            ...
            case sock_evt_disconnected:
            {
                ...
                if( userClient ){
                    ...
                    INIT_SOCKET_NOTIFICATION( notification, soObj, NkeSocketFilterEventDisconnected );
            ...
            case sock_evt_shutdown:
            {
                ...
                if( SHUT_RDWR == how ){
                    ...
                    //
                    soObj->reinjectDeferredData( NkeSocketObject::NkeSocketDataOutbound );
                    soObj->purgePendingQueue( NkeSocketObject::NkeSocketDataInbound );
            ...
            case sock_evt_cantrecvmore:
            {
                ...
                // }
                //
                soObj->reinjectDeferredData( NkeSocketObject::NkeSocketDataInbound );
...
/*
 ...
 #0  NkeSocketFilter::FltDataIn (cookie=0x0, so=0x4f560f0, from=0x0, data=0x2a723b14, control=0x0, flags=0) at /work/DL_MacSvn/mac/dl-0.x.beta/mac/dl-0.x/NKEriver/NKE/NkeSocketFilter.cpp:246
 ...
 */
errno_t	
NkeSocketFilter::FltDataIn(void *cookie, socket_t so, const struct sockaddr *from,
...
/*
 ...
 #0  NkeSocketFilter::FltDataOut (cookie=0x0, so=0x4f5650c, to=0x0, data=0x30cbbe0c, control=0x30cbbe10, flags=0) at /work/DL_MacSvn/mac/dl-0.x.beta/mac/dl-0.x/NKEriver/NKE/NkeSocketFilter.cpp:253
 ...
 #5  0x0054e9fd in unix_syscall64 (state=0x7c3feb4) at /work/Mac_OS_X_kernel/10_6_4/xnu-1504.7.4/bsd/dev/i386/systemcalls.c:365 
    ...
    @typedef NkeSocketFilter::FltDataOut
...
/*
 ...
 #0  NkeSocketFilter::FltConnectIn (cookie=0x0, so=0x6100160, from=0x31c73b68) at /work/DL_MacSvn/mac/dl-0.x.beta/mac/dl-0.x/NKEriver/NKE/NkeSocketFilter.cpp:259
 ...
 */
errno_t	
NkeSocketFilter::FltConnectIn(void *cookie, socket_t so, const struct sockaddr *from)
{
...
/*
 ...
 #0  NkeSocketFilter::FltConnectOut (cookie=0x0, so=0xc40c1d0, to=0x2c17bea8) at /work/DL_MacSvn/mac/dl-0.x.beta/mac/dl-0.x/NKEriver/NKE/NkeSocketFilter.cpp:265
 ...
 */
errno_t	
NkeSocketFilter::FltConnectOut(void *cookie, socket_t so, const struct sockaddr *to)
{
...
/*
 ...
 #0  NkeSocketFilter::FltBind (cookie=0x0, so=0x6100160, to=0x322d3e88) at /work/DL_MacSvn/mac/dl-0.x.beta/mac/dl-0.x/NKEriver/NKE/NkeSocketFilter.cpp:271
 ...
 */
errno_t	
NkeSocketFilter::FltBind(void *cookie, socket_t so, const struct sockaddr *to)
{
    NkeSocketObject* soObj = NkeCookieToSocketObject( cookie );
...
errno_t	
NkeSocketFilter::FltSetoption(void *cookie, socket_t so, sockopt_t opt)
{
    return KERN_SUCCESS;
}
...
errno_t
NkeSocketFilter::FltGetoption(void *cookie, socket_t so, sockopt_t opt)
{
    return KERN_SUCCESS;
}
...
/*
 ...
 #0  NkeSocketFilter::FltListen (cookie=0x0, so=0x6100160) at /work/DL_MacSvn/mac/dl-0.x.beta/mac/dl-0.x/NKEriver/NKE/NkeSocketFilter.cpp:289
 ...
 */
errno_t	
NkeSocketFilter::FltListen(void *cookie, socket_t so)
{
    return KERN_SUCCESS;
}
...
IOReturn NkeSocketFilter::registerUserClient( __in NkeIOUserClient* client )
{
...
IOReturn NkeSocketFilter::unregisterUserClient( __in NkeIOUserClient* client )
{
...
bool NkeSocketFilter::isUserClientPresent()
{
...
//
NkeIOUserClient* NkeSocketFilter::getUserClient()
{
...
void NkeSocketFilter::releaseUserClient()
{
...
IOMemoryDescriptor* NkeSocketFilter::getSocketBufferMemoryDescriptor( __in UInt32 index )
{
    assert( index < kt_NkeSocketBuffersNumber );
    
    if( index >= this->dataBuffers->getCount() )
        return NULL;
    
    NkeDataBuffer*  dataBuffer;
    
    dataBuffer = OSDynamicCast( NkeDataBuffer, this->dataBuffers->getObject( index ) );
    assert( dataBuffer );
...
//
NkeDataBuffer* NkeSocketFilter::acquireSocketDataBufferForIO()
{
    //
    ...
    do{
        ...
        assert( UINT8_MAX == freeIndex || freeIndex < NKE_STATIC_ARRAY_SIZE( this->freeBuffers ) );
    ...
    dataBuffer = OSDynamicCast( NkeDataBuffer, this->dataBuffers->getObject( freeIndex ) );
    assert( dataBuffer );
...
//
void NkeSocketFilter::releaseSocketDataBufferFromIO( __in UInt32 index )
{
#if DBG
    UInt8 nextFreeBuffer = this->freeBuffers[ index ];
    assert( index < kt_NkeSocketBuffersNumber && UINT8_MAX == nextFreeBuffer );
#endif
    
    if( index >= kt_NkeSocketBuffersNumber || UINT8_MAX != this->freeBuffers[ index ] ){
...
//
errno_t
NkeSocketFilter::copyDataToBuffers(
    ...
    )
{
    ...
    //
    if( i < kt_NkeSocketBuffersNumber )
    ...
    if( error ){
        ...
        // calls NkeSocketObject::DeliverWaitingNotifications() that
...
void
NkeSocketFilter::releaseDataBuffers(
    __inout UInt8*    bufferIndices // an array bufferIndices[ kt_NkeSocketBuffersNumber ]
    )
{
    for( int i = 0x0; i < kt_NkeSocketBuffersNumber; ++i ){
...
void
NkeSocketFilter::releaseDataBuffersAndDeliverNotifications(
    __inout UInt8*    bufferIndices // an array bufferIndices[ kt_NkeSocketBuffersNumber ]
    )
{
    this->releaseDataBuffers( bufferIndices );
    ...
    )
{
    ...
    //
    assert( NKE_STATIC_ARRAY_SIZE( response->buffersToRelease ) == kt_NkeSocketBuffersNumber );
    ...
    //
    for( int i = 0x0; i < NKE_STATIC_ARRAY_SIZE( response->property ); ++i ){
        
        NkeSocketDataProperty*   property = &response->property[ i ];
        
        if( NkeSocketDataPropertyTypeUnknown == property->type )
            break;
        ...
        //
        NkeSocketObject* soObj = NkeSocketObject::GetSocketObjectRef( (socket_t)property->socketId.socket );
        if( soObj ){
